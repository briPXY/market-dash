...
export function drawMACD(d3, svg, data, scale, color, id, dim) {
    ...
    const yMin = d3.min(data, d => Math.min(d.MACD, d.signal, d.histogram));
    const yMax = d3.max(data, d => Math.max(d.MACD, d.signal, d.histogram));

    const yScale = d3.scaleLinear()
        .domain([yMin, yMax])
        .range([dim.h - dim.m.bottom, dim.m.top]); // Inverted for SVG coordinate system
